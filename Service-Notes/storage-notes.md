...
- EBS(Elastic Block Store): acts as a dedicated, persistent hard drive for a single cloud server. It is a block storage, which means the data is stored in individual, unformatted blocks, just like a traditional hard disk. It can be attached to only one EC2 instance at a time. The data remains even if the EC2 instance is stopped or terminated, making it a persistent storage.

- EFS(Elastic File System): is file storage that acts as a traditional shared file system, allowing you to organize data into folders and files similar to a network-attached storage or NAS.
...
- Amazon EFS is used for web applications that need to share the same files such a website images or logs, development environments where multiple delopers need access to the same codebase, and Content Management Systems.
